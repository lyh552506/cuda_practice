...
__global__ void copy_global_shm_register(const T *Aptr) {
  ...
  auto gA = make_tensor(make_gmem_ptr(Aptr), make_shape(Int<M>{}, Int<N>{}),
  ...
  // error: In CopyAtom, src layout doesn't vectorize into registers. This src
  // layout is incompatible with this tiled copy. auto stAsA =
  // s2r_thr_copy.partition_S(sA);
  ...
  if (idx == 0) {
    // ((_8,_1),_4,_4)
    // (CPY, CPY_M, CPY_N)
    // 其中CPY由copy_op決定，這裡對2個OP都是128bit=16bytes=8half,
    // 確定了copy的基本操作單位。 CPY_M = M / (ThrLayout_M * ValLayout_M), CPY_N
    // = N / (ThrLayout_N * ValLayout_N),
    // CPY_M、CPY_N和TiledCopy共同確定了拷貝的元素數量。
...
int main() {
  ...
  err = cudaDeviceGetAttribute(&sharedMemPerBlock,
  ...
  auto testt=make_tiled_copy(g2s_copy_atom{},
	make_layout(make_shape(Int<32>{}, Int<4>{}),
				make_stride(Int<4>{}, Int<1>{})),
	make_layout(make_shape(Int<1>{}, Int<8>{})));
  ...
  ldmatrix由于是单线程提供16Byte的数据地址，warp内所有线程可以提供512Byte的数据到寄存器的加载，单指令实现16x16
  float16矩阵的加载，减少指令数。
  */
  ...
  for (int i = 0; i < count; ++i) {
    copy_global_shm_register<T, G2SCopy, S2RCopy, SmemLayout, M, N>
        <<<1, block, shm_size>>>(Aptr);
  }
  cudaEventRecord(end);
  ...
  std::cout << "copy_global_shm_register took " << elapsedTime / count << "ms."
            << std::endl;
}